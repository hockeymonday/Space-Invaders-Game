...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        self._text = GLabel(x=400 ,y=350,text='Press the "F" key to begin',
            font_size = 40, font_name = 'RetroGame.ttf', halign = 'center',
            valign = 'top')
        self._wave = None
    ...
    def update(self,dt):
        ...
        assert type(dt) == int or type(dt) == float,'dt must be int or float'

        #Calls helper method to update for the following states
        if self._state == STATE_INACTIVE:
            self.state_inactive_update()
    ...
    def state_inactive_update(self):
        """
        Updates the logic of a single frame in the state: STATE_INACTIVE.
        Remains in this state until 'F' key is pressed and then becomes the
        STATE_NEWWAVE state the next time the frame is updated.

        STATE_INACTIVE: This is the state when the application first opens.
        It is a paused state, waiting for the player to start the game.  It
        displays a simple message on the screen. The application remains in
        this state so long as the player never presses the 'F' key. In addition,
        this is the state the application returns to when the game is over
        (all lives are lost or all aliens are dead). If the 'F' key is pressed
        then the state becomes the newwave state (STATE_NEWWAVE).
        """
        pressed = self.input.is_key_down('f')
        if pressed == True:
            #Now state is STATE_NEWWAVE for 1 frame before it switches again
